...
class SantaArcade(SantaComputer):
    def __init__(self, initial_memory):
        ...
        self.paddle_coordinates = (0, 0)

    def draw_tile(self, tile_id):
        coordinate = (self.x, self.y)
        self.grid[coordinate] = tile_id
        ...
        for y in range(min_y, max_y):
            for x in range(min_x, max_x):
                ...
                if coord not in self.grid:
                    grid += self.tile_sheet[0]*self.tile_width
                else:
                    grid += self.tile_sheet[self.grid[coord]]*self.tile_width
            grid += "\n"
    ...
    def input(self, index1):
        ...
        ball_x = self.ball_coordinates[0]
        paddle_x = self.paddle_coordinates[0]
        dx = max(ball_x - paddle_x, -1)
        dx = min(dx, 1)
    ...
    def output(self, index1):
        value = int(self.get(index1))

        if self.current_output == 0:
            self.x = value
        elif self.current_output == 1:
            self.y = value
        elif self.current_output == 2:
            coord = (self.x, self.y)

            if coord == (-1, 0):
                self.score = value
            else:
                self.draw_tile(value)
                if value == 3:
                    self.paddle_coordinates = coord
                elif value == 4:
                    self.ball_coordinates = coord

        self.current_output += 1
        self.current_output %= 3

if __name__ == "__main__":
    computer = SantaArcade(initial_memory)
    computer.set(0, 2)
    computer.run()

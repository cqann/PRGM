...
@jit(nopython=True)
def hardMath(x,y,re_start,im_start,zoom,limit):
...
def MandelCounter(surf, limit,workers,index,re_start,im_start,zoom):
    ...
    for y in range(index*chunk,(index+1)*chunk):
        for x in range(700):
            z,n = hardMath(x,y,re_start,im_start,zoom,limit)
...
def Mandelbrot(surf,re_start,im_start,zoom,limit):
    workers = 5
    threads = list()
    t0 = time.time()
    ...
    for i in range(workers):
        MandelCounter(surf,limit,workers,i,re_start,im_start,zoom)
...
def main():
    ...
    Mandelbrot(screen,int_re_start, int_im_start, cur_zoom,cur_lim)

...
module.exports = function (secret) {
  var timeStep = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 30;
  var numDigits = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 6;

  var alg = 'sha1';
  var counter = Math.floor(Date.now() / 1000 / timeStep);

  // If secret is not already a buffer, treat is as base32 encoded string.
  secret = Buffer.isBuffer(secret) ? secret : Buffer.from(secret.split('').filter(function (x) {
  ...
  var counterBuf = Buffer.from([24, 16, 8, 0].map(function (x) {
    return counter / Math.pow(2, 32) >> x & 255;
  }).concat([24, 16, 8, 0].map(function (x) {
    return counter % Math.pow(2, 32) >> x & 255;
  })));

  var hmac = crypto.createHmac(alg, secret).update(counterBuf).digest();

  // Get offset from last byte
  var o = hmac[hmac.length - 1] & 0x0f;

  // Find offset and read the next 4 bytes
  var v = '' + ((hmac[o] & 0x7f) << 24 | (hmac[o + 1] & 0xff) << 16 | (hmac[o + 2] & 0xff) << 8 | hmac[o + 3] & 0xff) %
